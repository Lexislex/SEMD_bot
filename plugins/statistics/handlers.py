...
class StatisticsHandlers:
    ...
    def handle_stat_menu(self, call: CallbackQuery):
        """Handle menu button click for Statistics plugin - show statistics"""
        try:
            # Check admin access
            ...
            if not activity_data:
                ...
                self.bot.edit_message_text(
                    ...
                    text="📊 Нет данных активности за указанный период.",
                    reply_markup=markup
                )
            ...
            if df.empty:
                ...
                self.bot.edit_message_text(
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    text="📊 Нет данных активности от пользователей за указанный период.",
                    reply_markup=markup
                )
                # Update tracked message to current one
                get_message_manager().update_message(call.message.chat.id, call.message.message_id, call.from_user.id)
